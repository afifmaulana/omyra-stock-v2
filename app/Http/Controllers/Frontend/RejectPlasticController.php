...
class RejectPlasticController extends Controller
{
    public function index()
    {
        ...
        return view('ui.frontend.reject.plastic.index',[
            'rejects' => $rejects,
        ]);
    }
    ...
    public function create()
    {
        ...
        return view('ui.frontend.reject.plastic.create',[
            'brands' => $brands,
            'products' => $products,
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            RecordLog::saveRecord($dataStock);
             ...
             $title = $description = Auth::user()->name . ' telah menambahkan plastik reject'.
             ...
             return redirect()->route('frontend.reject.plastic.index')->with(['success' => 'Data baru berhasil ditambahkan.']);
         } catch (\Throwable $th) {
             DB::rollBack();
             dd($th->getMessage());
         }
    ...
    public function destroy($id)
    {
        ...
        $reject->delete();
            ...
            $title = $description = Auth::user()->name . ' telah menghapus stok plastik dengan ID #'. $reject->id;
            $log = new LogActivity();
            $log->user_id = Auth::user()->id;
            $log->source_id = $reject->id;
            $log->source_type = '\App\Reject';
            ...
            $log->description = $description;
            $log->save();
        return redirect()->route('frontend.reject.plastic.index')->with(['success' => 'Berhasil menghapus data.']);
    }
}
