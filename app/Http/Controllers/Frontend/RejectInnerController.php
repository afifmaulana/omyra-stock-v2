...
class RejectInnerController extends Controller
{
    public function index()
    {
        ...
        return view('ui.frontend.reject.inner.index',[
            'rejects' => $rejects,
        ]);
    }
    ...
    public function create()
    {
        ...
        return view('ui.frontend.reject.inner.create',[
            'brands' => $brands,
            'products' => $products,
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            RecordLog::saveRecord($dataStock);
                ...
                $title = $description = Auth::user()->name . ' telah menambahkan inner reject'.
                ...
                return redirect()->route('frontend.reject.inner.index')->with(['success' => 'Data baru berhasil ditambahkan.']);
            } catch (\Throwable $th) {
                DB::rollBack();
                dd($th->getMessage());
            }
    ...
    public function destroy($id)
    {
        ...
        $reject->delete();
            ...
            $title = $description = Auth::user()->name . ' telah menghapus stok inner dengan ID #'. $reject->id;
            $log = new LogActivity();
            $log->user_id = Auth::user()->id;
            $log->source_id = $reject->id;
            $log->source_type = '\App\Reject';
            ...
            $log->description = $description;
            $log->save();
        return redirect()->route('frontend.reject.inner.index')->with(['success' => 'Berhasil menghapus data.']);
    }
}
